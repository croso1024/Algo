...
class TabuSearch:
    ...
    def reset(self):
        ...
        #由於定義的鄰近解是某一組解的組合中任意選出兩個來交換
        # combination的第一個引數為iterable obj, 直接range(3)相當於給[0,1,2]
        # combination會回傳一組type為itertool.combination object,用list來得到組合的結果
        #self.candidate_swapped_index = list(combinations(list(range(self.var_num)), 2))
        self.candidate_swapped_index = list(combinations(range(self.var_num), 2))
        print(self.candidate_swapped_index)
